...
(defun display-frame ()
  (let* ((moving-list nil)
         (this-turn-tree (force (game-tree *player-a* 0 '() (copy-seq *default-board*))))

         (handle-func1 'handle-human)
         (handle-func2 'handle-human)
         (handle-func handle-func1)
         (handled-tree nil)
         (computer-thinking? nil)
         (want-player-input? nil))
    (with-ltk ()
      (let* (;; widgets ;;;;;;;;;;;;;;
              ...
              (setq handled-tree
                    (reduce #'(lambda (x y)
                                (cond (x x)
                                      ...
                                      (t nil)))
                            (game-tree-tree this-turn-tree) :initial-value nil))))
         

         (if handled-tree (setq this-turn-tree (force handled-tree)))
